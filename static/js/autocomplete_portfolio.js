...
$(document).ready(function(){
    async function allCompany(url) {
        try {
            ...
            //   ~async function() {
            //     let response = await fetch(".").then(resp => resp.text())
    ...
    /*check if the item starts with the same letters as the text field value:*/
        if (val.length == 1) {
            if (arr[i].substr(0,1).toUpperCase().includes(val.toUpperCase()) & (count < 14))
                ...
                b.innerHTML += "<div><button data-value='"+ticker[0]+"' type='submit' class='btn btn-sm ml-3 border-0 bg-light button__submit__portfolio' value='+'>+</button></div>";
                ...
                function clickTarget(e) {
                    ...
                    if (e.target.nodeName == "BUTTON") {
                            ...
                            metaDate = { 
                                symbols: $(this).find('input').val().split(' | ')[0],
                                csrf_token: $('#csrf').val()
                            }
                ...
                b.addEventListener("click", clickTarget, false);
        ...
        if (arr[i].substr(0,2).toUpperCase().includes(val.toUpperCase()) & (count < 14))
            {
                ...
                // b.innerHTML = arr[i]
                b.appendChild(xxx)
                // console.log(getElementsByClassName('tickerCompany'));
                b.innerHTML += "<input type='hidden' value='" + arr[i] + "'>";
                b.innerHTML += "<div><button type='submit' class='btn btn-sm ml-3 border-0 bg-light' value='+' id='button__submit__portfolio'>+</button></div>";
